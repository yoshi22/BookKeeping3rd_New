...
describe('QuestionRepository', () => {
  ...
  const sampleQuestions: Question[] = [
    {
      ...
      answer_template_json: JSON.stringify({
        type: 'journal_entry',
        fields: [
          { label: '借方勘定科目', type: 'dropdown', name: 'debit_account', required: true },
          { label: '借方金額', type: 'number', name: 'debit_amount', required: true }
        ]
      }),
    ...
    },
    {
      ...
      answer_template_json: JSON.stringify({
        type: 'ledger_entry',
        fields: [
          { label: '摘要', type: 'dropdown', name: 'description', required: true },
  ...
  describe('学習状況による検索', () => {
    test('findUnstudiedQuestions()が未学習問題を返す', async () => {
      ...
      const result = await questionRepository.findUnstudiedQuestions('journal', 5);
    ...
    test('findReviewQuestions()が復習対象問題を優先度順で返す', async () => {
      ...
      const result = await questionRepository.findReviewQuestions('journal', 10);
  ...
  describe('コンテンツ構成検証', () => {
    ...
    test('validateContentStructure()が不正な構成を検出する', async () => {
      ...
      expect(validation.issues.some(issue => issue.includes('journal'))).toBe(true);
    });
  });
