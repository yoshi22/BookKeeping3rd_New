...
export class ReviewItemRepository extends BaseRepository<ReviewItem> {
  ...
  public async createOrUpdate(data: CreateReviewItemData): Promise<ReviewItem> {
    try {
      ...
      console.log(`[ReviewItemRepository] 既存アイテム検索結果:`, existing);

      if (existing) {
        // 既存レコードの更新
        console.log(`[ReviewItemRepository] 既存レコード更新処理`);
        const updateData: UpdateReviewItemData = {
          incorrectCount: data.incorrectCount ?? existing.incorrect_count,
          consecutiveCorrectCount:
            data.consecutiveCorrectCount ?? existing.consecutive_correct_count,
        ...
        // 新規作成
        console.log(`[ReviewItemRepository] 新規アイテム作成処理`);
        const reviewItemData = {
          ...
          priority_score:
            data.priorityScore ??
            this.calculateInitialPriority(data.incorrectCount ?? 1),
        ...
        console.log(`[ReviewItemRepository] 新規作成データ:`, reviewItemData);
        const result = await this.create(reviewItemData);
        console.log(`[ReviewItemRepository] 新規作成結果:`, result);
        return result;
      }
    } catch (error) {
      console.error("[ReviewItemRepository] createOrUpdate エラー:", error);
      console.error("[ReviewItemRepository] Error details:", {
  ...
  public async updateByQuestionId(
    ...
    try {
      ...
      if (updateData.consecutiveCorrectCount !== undefined) {
        updateFields.consecutive_correct_count =
          updateData.consecutiveCorrectCount;
      }
      ...
      const result = await this.updateWhere(
  ...
  public async getReviewList(filter: ReviewFilter = {}): Promise<ReviewItem[]> {
    try {
      console.log(`[ReviewItemRepository] getReviewList開始:`, filter);
      let sql = `
      ...
      console.log(`[ReviewItemRepository] 実行SQL:`, sql);
      console.log(`[ReviewItemRepository] SQLパラメータ:`, params);
  ...
  public async getReviewStatistics(): Promise<ReviewStatistics> {
    try {
      ...
      const priorityDistResult = await this.executeQuery<any>(
        priorityDistQuery,
        [],
      );
      ...
      priorityDistResult.rows.forEach((row) => {
        priorityDistribution[
          row.priority_level as keyof typeof priorityDistribution
        ] = row.count;
      });
   ...
   * 克服済み問題のクリーンアップ
   */
  public async cleanupMasteredItems(
