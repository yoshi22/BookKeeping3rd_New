...
export class StatisticsCacheService {
  ...
  public invalidateOnAnswerSubmit(): void {
    logger.debug("[StatisticsCache] 解答記録によるキャッシュ無効化");
  ...
  public invalidateCategory(categoryId: string): void {
    logger.debug(
      "[StatisticsCache] カテゴリ特定キャッシュ無効化: ${categoryId}",
    );
  ...
  public invalidateOnReviewUpdate(): void {
    logger.debug("[StatisticsCache] 復習データ更新によるキャッシュ無効化");
  ...
  public clearAll(): void {
    logger.debug("[StatisticsCache] 全キャッシュクリア");
    this.cache.clear();
  }
  ...
  private getTTL(key: CacheKey): number {
    ...
    return (
      this.cacheConfig[key as keyof typeof this.cacheConfig] ||
  ...
  private cleanupExpiredEntries(): void {
    ...
    if (expiredKeys.length > 0) {
      logger.debug(
        "[StatisticsCache] 期限切れエントリ削除: ${expiredKeys.length}件",
      );
    }
  ...
  private invalidateDailyStatsForToday(): void {
    const keysToCheck = Array.from(this.cache.keys()).filter(
      (key) => typeof key === "string" && key.startsWith("daily_stats_"),
    );
