...
export class StatisticsService {
  ...
  public async getCategoryStatistics(): Promise<CategoryStatistics[]> {
    try {
      ...
      const categories: QuestionCategory[] = ['journal', 'ledger', 'trial_balance'];
      const categoryNames = {
        journal: '仕訳',
        ledger: '帳簿',
        trial_balance: '試算表'
   ...
   * 日別学習統計取得
   */
  public async getDailyStatistics(days: number = 30): Promise<DailyStatistics[]> {
    try {
  ...
  public async getLearningTrends(): Promise<LearningTrends> {
    try {
      ...
      const consistencyScore = this.calculateConsistencyScore(await this.getDailyStatistics(30));
    ...
    try {
      ...
      const goals: LearningGoals = {
        ...
        accuracyGoal: {
          ...
          achievement: currentAccuracy >= targets.accuracy ? 'achieved' :
                      currentAccuracy >= targets.accuracy - 0.1 ? 'close' : 'needs_improvement'
        }
      };
   ...
   * カテゴリ別問題数取得
   */
  private async getCategoryQuestionsCount(category: QuestionCategory): Promise<number> {
    const result = await questionRepository.executeQuery<{ count: number }>(
    ...
    // デフォルト値（仕様書ベース）
    const defaults = { journal: 250, ledger: 40, trial_balance: 12 };
  ...
  private async calculateStudyDays(): Promise<number> {
    const result = await learningHistoryRepository.executeQuery<{ days: number }>(
   ...
   * 連続学習日数計算
   */
  private async calculateStudyStreaks(): Promise<{ currentStreak: number; maxStreak: number }> {
    const result = await learningHistoryRepository.executeQuery<{ date: string }>(
   ...
   * 学習期間取得
   */
  private async getStudyDateRange(): Promise<{ firstStudiedAt?: string; lastStudiedAt?: string }> {
    const result = await learningHistoryRepository.executeQuery<{ first: string; last: string }>(
  ...
  private async getDifficultyBreakdown(category: QuestionCategory) {
    ...
    result.rows.forEach(row => {
      const level = row.difficulty <= 2 ? 'easy' : row.difficulty <= 3 ? 'medium' : 'hard';
   ...
   * 克服済み問題数取得
   */
  private async getMasteredQuestionsCount(category: QuestionCategory): Promise<number> {
    const result = await learningHistoryRepository.executeQuery<{ count: number }>(
   ...
   * 正答率傾向分析
   */
  private analyzeAccuracyTrend(weeklyData: any[]): 'improving' | 'stable' | 'declining' {
    // 実装省略 - 正答率の傾向分析
    return 'stable';
  }
  ...
  private calculateConsistencyScore(dailyStats: DailyStatistics[]): number {
    const studyDays = dailyStats.filter(day => day.questionsAnswered > 0).length;
  ...
  private generateRecommendations(trends: {
    ...
    if (trends.consistencyScore < 50) {
      recommendations.push('毎日少しずつでも学習を継続することを心がけましょう');
    }
