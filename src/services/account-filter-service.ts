...
} from "@/components/shared/AccountOptions";
import {
  AccountCategory,
...
export class AccountFilterService {
  ...
  private performThreeStageFiltering(
    ...
    relatedAccounts.forEach((account) => {
      if (
        accountSet.size < maxAccounts - 1 &&
        !excludeAccounts.includes(account)
      ) {
        // デフォルトオプション分を除く
        accountSet.add(account);
      }
    ...
    if (accountSet.size < Math.min(10, maxAccounts - 1)) {
      const supplementaryAccounts = this.getSupplementaryAccounts(
        category,
  ...
  private getRelatedAccounts(
    ...
    if (questionId && GENERATED_QUESTION_ACCOUNT_MAPPINGS[questionId]) {
      GENERATED_QUESTION_ACCOUNT_MAPPINGS[questionId].relatedAccounts.forEach(
        (account) => relatedAccounts.add(account),
      );
    }
    ...
    if (categoryDef) {
      categoryDef.coreAccounts.forEach((account) =>
        relatedAccounts.add(account),
      );
   ...
   * Stage 3: 補完科目の取得
   */
  private getSupplementaryAccounts(
    category: AccountCategory,
