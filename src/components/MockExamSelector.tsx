...
export const MockExamSelector: React.FC<MockExamSelectorProps> = ({
    ...
    try {
      ...
      const enrichedExams: MockExamItemData[] = await Promise.all(
        exams.map(async (exam) => {
          ...
          const lastResult = await mockExamService.getLatestMockExamResult(exam.id);
    ...
    <View style={styles.examCard}>
      ...
      <View style={styles.examStats}>
        ...
        <View style={styles.statItem}>
          ...
          <Text style={styles.statValue}>{item.attemptCount}回</Text>
        </View>
        ...
        {item.bestScore > 0 && (
          <View style={styles.statItem}>
            ...
            <Text style={[
              styles.statValue,
              item.bestScore >= item.passing_score ? styles.passedScore : styles.failedScore
            ]}>
              {item.bestScore}点
            </Text>
          </View>
      ...
      {item.lastResult && (
        <View style={styles.lastResultContainer}>
          ...
          <Text style={[
            ...
            item.lastResult.is_passed ? styles.passedScore : styles.failedScore
          ]}>
      ...
      <View style={styles.actionButtons}>
        ...
        {item.attemptCount > 0 && onViewResults && (
          <TouchableOpacity
  ...
  const getDifficultyLevel = (examId: string): string => {
    switch (examId) {
      case 'MOCK_001': return '基礎';
      case 'MOCK_002': return '標準';
      case 'MOCK_003': return '応用';
      case 'MOCK_004': return '実践';
      case 'MOCK_005': return '総合';
      default: return '標準';
    }
  };
