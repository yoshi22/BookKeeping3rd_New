...
export function useAppInitialization() {
  ...
  const initializeApp = useCallback(async () => {
    ...
    try {
      ...
      logger.debug("[AppInit] Stage 2: 必須データ読み込み開始");
      ...
      logger.debug("[AppInit] Stage 3: バックグラウンドタスク開始");
      ...
      logger.error("[AppInit] 初期化エラー:", error as Error);
      setState((prev) => ({
        ...prev,
        stage: "error",
  ...
  const warmupStatisticsCache = async () => {
    try {
      ...
      logger.debug("[AppInit] 統計キャッシュ温機完了");
    } catch (error) {
      logger.warn("[AppInit] 統計キャッシュ温機エラー:", { details: error });
    }
  };
  ...
  const preloadRecentLearningData = async () => {
    try {
      ...
      logger.debug("[AppInit] 最近の学習データプリロード完了");
    } catch (error) {
      logger.warn("[AppInit] 学習データプリロードエラー:", { details: error });
    }
  };
  ...
  const optimizeDatabaseInBackground = async () => {
    try {
      ...
      logger.debug("[AppInit] データベース最適化完了");
    } catch (error) {
      logger.warn("[AppInit] データベース最適化エラー:", { details: error });
    }
  };
