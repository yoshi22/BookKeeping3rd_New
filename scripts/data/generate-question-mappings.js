  ...
  try {
    ...
    // 帳簿問題の場合
      ...
      answer.entries.forEach((entry) => {
        if (
          entry.description &&
          entry.description !== "前月繰越" &&
          entry.description !== "次月繰越"
        ) {
...
// 補完科目を生成
function generateSupplementaryAccounts(
  category,
  primaryAccounts,
  relatedAccounts,
) {
...
function generateMappings() {
  ...
  for (const question of questions) {
    try {
      // 基本情報の抽出
      const primaryAccounts = extractAccountsFromAnswer(
      ...
      // 関連科目・補完科目の生成
      const relatedAccounts = generateRelatedAccounts(
        category,
        primaryAccounts,
      );
      const supplementaryAccounts = generateSupplementaryAccounts(
        category,
        primaryAccounts,
        relatedAccounts,
      );
      ...
      mappings[question.id] = {
        ...
        relatedAccounts: relatedAccounts.filter(
          (acc) => !primaryAccounts.includes(acc),
        ),
  ...
  for (const mapping of Object.values(mappings)) {
    stats.categoryCount[mapping.category] =
      (stats.categoryCount[mapping.category] || 0) + 1;
    totalPrimary += mapping.primaryAccounts.length;
    totalRelated += mapping.relatedAccounts.length;
  }

  stats.avgPrimaryAccounts =
    Math.round((totalPrimary / processedCount) * 10) / 10;
  stats.avgRelatedAccounts =
    Math.round((totalRelated / processedCount) * 10) / 10;
 ...
 */
export const GENERATED_QUESTION_ACCOUNT_MAPPINGS: Record<string, QuestionAccountMapping> = ${JSON.stringify(
    mappings,
    null,
    2,
  ).replace(/"category": "([^"]+)"/g, (match, categoryName) => {
    const categoryMap = {
      cash_deposit: "CASH_DEPOSIT",
      merchandise: "MERCHANDISE",
      fixed_assets: "FIXED_ASSETS",
