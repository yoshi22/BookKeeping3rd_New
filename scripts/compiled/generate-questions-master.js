...
function getCategoryLayer1(subcategory) {
    const categoryMap = {
...
function getCategoryLayer2(subcategory, patternName) {
    const patternMap = {
        cash_deposit: {
...
function generateJournalQuestions() {
    ...
    for (const subcategory of orderedSubcategories) {
        const subcategoryConfig = QUESTION_TARGETS.journal.subcategories[subcategory];
        const templates = JOURNAL_TEMPLATES[subcategory];
        if (!subcategoryConfig || !templates)
            continue;
        ...
        for (let i = 0; i < targetCount; i++) {
            ...
            if (!pattern)
                continue;
            const difficulty = calculateDifficulty(questionIndex - 1, totalQuestions, DIFFICULTY_DISTRIBUTION.journal);
            // 金額を生成
...
function validateQuestions(questions) {
    ...
    for (const [sub, target] of Object.entries(QUESTION_TARGETS.journal.subcategories)) {
        ...
        if (actual !== target.total) {
            result.errors.push(`仕訳/${sub}の問題数不一致: 期待${target.total}問、実際${actual}問`);
            result.valid = false;
        }
    }
    // 帳簿問題
