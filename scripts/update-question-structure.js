...
const database_1 = require("../src/data/database");
...
async function updateQuestionStructure() {
    ...
    try {
        ...
        if (!database_1.databaseService.isConnected()) {
            throw new Error("データベース接続失敗");
        }
        console.log("問題データ取得中...");
        const questions = await database_1.databaseService.executeSql("SELECT id, category_id, difficulty, tags_json FROM questions ORDER BY category_id, id");
        console.log(`取得問題数: ${questions.rows.length}件`);
        // カテゴリ別の問題を分類
        const journalQuestions = questions.rows.filter((q) => q.category_id === "journal");
        const ledgerQuestions = questions.rows.filter((q) => q.category_id === "ledger");
        const trialBalanceQuestions = questions.rows.filter((q) => q.category_id === "trial_balance");
        ...
        // 更新結果の確認
        const updatedQuestions = await database_1.databaseService.executeSql(`
    ...
    `);
        ...
        for (const row of updatedQuestions.rows) {
            console.log(`${row.category_id} / ${row.subcategory} / ${row.pattern_type}: ${row.count}問`);
        }
    }
    catch (error) {
...
async function updateJournalQuestions(questions) {
    console.log("\n第1問（仕訳）の構造更新中...");
    let questionIndex = 0;
    let globalOrder = 1;
    // 各サブカテゴリを順番に処理
    for (const [subcategoryKey, subcategoryName] of Object.entries(JOURNAL_SUBCATEGORIES)) {
        console.log(`\n処理中: ${subcategoryName}`);
        const patterns = JOURNAL_PATTERNS[subcategoryKey] || [];
        for (const patternInfo of patterns) {
            ...
            for (let i = 0; i < count && questionIndex < questions.length; i++) {
                ...
                await database_1.databaseService.executeSql(`
          UPDATE questions 
          SET subcategory = ?, 
              section_number = 1,
              question_order = ?,
...
async function updateLedgerQuestions(questions) {
    console.log("\n第2問（帳簿）の構造更新中...");
    let questionIndex = 0;
    let globalOrder = 1;
    // 各サブカテゴリを順番に処理
    for (const [subcategoryKey, subcategoryName] of Object.entries(LEDGER_SUBCATEGORIES)) {
        console.log(`\n処理中: ${subcategoryName}`);
        const patterns = LEDGER_PATTERNS[subcategoryKey] || [];
        for (const patternInfo of patterns) {
            const { pattern, count, tags } = patternInfo;
            console.log(`  パターン: ${pattern} (${count}問)`);
            // 指定された問題数分を更新
...
async function updateTrialBalanceQuestions(questions) {
    console.log("\n第3問（試算表）の構造更新中...");
    let questionIndex = 0;
    let globalOrder = 1;
    // 各サブカテゴリを順番に処理
    for (const [subcategoryKey, subcategoryName] of Object.entries(TRIAL_BALANCE_SUBCATEGORIES)) {
        console.log(`\n処理中: ${subcategoryName}`);
        const patterns = TRIAL_BALANCE_PATTERNS[subcategoryKey] || [];
        for (const patternInfo of patterns) {
            const { pattern, count, tags } = patternInfo;
            console.log(`  パターン: ${pattern} (${count}問)`);
            // 指定された問題数分を更新
