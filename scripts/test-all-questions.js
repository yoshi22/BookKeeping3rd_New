 ...
 * - テンプレートJSONの妥当性チェック
 * - 正答JSONの形式確認
 * - Answer Serviceでの判定テスト
 */

const fs = require("fs");
const path = require("path");

// TypeScriptファイルを直接読み込む
...
function extractQuestions() {
  const questions = [];
  const regex =
    /"id":\s*"([^"]+)"[\s\S]*?"category_id":\s*"([^"]+)"[\s\S]*?"question_text":\s*"([^"]*)"[\s\S]*?"answer_template_json":\s*"([^"]*)"[\s\S]*?"correct_answer_json":\s*"([^"]*)"[\s\S]*?"tags_json":\s*"([^"]*)"/g;
  ...
  while ((match = regex.exec(content)) !== null) {
    questions.push({
      id: match[1],
      category_id: match[2],
      question_text: match[3].replace(/\\n/g, "\n").replace(/\\/g, ""),
      answer_template_json: match[4],
      correct_answer_json: match[5],
      tags_json: match[6],
    });
  }
  return questions;
...
function simulateAnswerCheck(question, template, correctAnswer) {
  // 実際のAnswer Serviceのロジックを簡略化して再現
  const results = {
    canInput: true,
    canJudge: true,
    errors: [],
...
async function main() {
  ...
  questions.forEach((question) => {
    ...
    // 1. テンプレートJSON検証
    const templateResult = validateJSON(
    ...
    // 2. 正答JSON検証
    const answerResult = validateJSON(
    ...
    // 3. タグJSON検証
    const tagsResult = validateJSON(question.tags_json, "tags_json");
    if (!tagsResult.valid) {
      hasError = true;
