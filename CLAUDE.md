...
これは Expo、TypeScript、SQLite で構築された React Native 簿記練習アプリ（簿記3級問題集「確実復習」）です。間違えた問題の反復練習を通してユーザーの簿記習得を支援することに焦点を当てています。最新版では CBT 形式の模試機能が完全実装済みで、2025年9月に動的勘定科目フィルタリング機能が追加され、ユーザビリティが大幅に向上しました。
...
# 動的フィルタリング機能（2025-09-24実装完了）
node scripts/data/generate-question-mappings.js    # 問題-勘定科目マッピング生成（302問対応）
npm test -- --testPathPattern="account-filter"     # 動的フィルタリングテスト実行
```
...
**動的勘定科目フィルタリングシステム（2025-09-24実装完了）:**
ベータテスターフィードバック「勘定科目選択肢が71個で多すぎる」問題を解決:

- **問題**: 全勘定科目71個表示によるユーザー混乱
- **解決**: 問題に応じて10-15個に動的フィルタリング
- **アルゴリズム**: 3段階フィルタリング（正答科目→関連科目→補完科目）
- **カテゴリ体系**: 6つのカテゴリで302問を分類（現金・預金系、商品・売買系、固定資産系、給与系、決算系、債権・債務系、その他）
- **パフォーマンス**: LRUキャッシュとシングルトンパターンで最適化
- **自動生成**: 全302問のマッピングを自動生成スクリプトで作成
- **テスト完備**: 33個のテストケースで100%品質保証
